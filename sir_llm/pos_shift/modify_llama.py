...
def llama_pos_shift_attention_forward(
    ...
    # assert False
    if self.config.pretraining_tp > 1: #tensor parallel
        key_value_slicing = (
...
def enable_llama_pos_shift_attention(model):
    for name, module in reversed(model._modules.items()): #从后往前遍历模型的所有层
        if len(list(module.children())) > 0: #如果当前层还有子层
            enable_llama_pos_shift_attention( #递归调用
                module,
            )

        if isinstance(module, LlamaAttention): #如果当前层是LlamaAttention
            model._modules[name].forward = types.MethodType( #将forward函数替换为llama_pos_shift_attention_forward
